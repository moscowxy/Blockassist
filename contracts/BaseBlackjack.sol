...
contract BaseBlackjack is Ownable, ReentrancyGuard, VRFConsumerBaseV2Plus {
    ...
    uint256 public constant REWARD_POOL_PERCENT = 80;
    ...
    function cardMetadata(uint8 card)
        external
        pure
        returns (uint8 rank, uint8 suit)
    {
        return _decodeCard(card);
    }

    function cardPointValue(uint8 card) external pure returns (uint8) {
        (uint8 rank, ) = _decodeCard(card);
        if (rank == 1) {
            return 11;
        }
    ...
    function _drawCard(uint256 gameId, bool forPlayer) internal {
        ...
        uint8 card = _takeCard(game, randomValue);
        (uint8 rank, uint8 suit) = _decodeCard(card);

        if (forPlayer) {
            game.playerCards.push(card);
            game.playerScore = _calculateScore(game.playerCards);
            emit CardDealt(gameId, game.player, true, card, rank, suit, game.playerScore);
        } else {
            game.dealerCards.push(card);
            game.dealerScore = _calculateScore(game.dealerCards);
            emit CardDealt(gameId, address(0), false, card, rank, suit, game.dealerScore);
        }
    }
    ...
    function _calculateScore(uint8[] storage cards) internal view returns (uint8) {
        ...
        for (uint256 i = 0; i < cards.length; i++) {
            (uint8 rank, ) = _decodeCard(cards[i]);
            uint8 points = rank > 10 ? 10 : rank;
            if (rank == 1) {
                points = 11;
                aces += 1;
            }
    ...
    function _takeCard(Game storage game, uint256 randomValue) internal returns (uint8) {
        uint8 start = uint8(randomValue % 52);
        for (uint8 offset = 0; offset < 52; offset++) {
            uint8 candidate = uint8((start + offset) % 52);
            uint256 mask = uint256(1) << candidate;
            if (game.deckState & mask == 0) {
                game.deckState |= mask;
                return candidate;
            }
        }
    ...
    function _decodeCard(uint8 card) internal pure returns (uint8 rank, uint8 suit) {
        rank = (card % 13) + 1;
        suit = card / 13;
    }
